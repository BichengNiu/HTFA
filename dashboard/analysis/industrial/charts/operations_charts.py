...
class OperationsIndicatorsChart(BaseChartCreator):
    ...
    def _prepare_data(
        ...
        available_indicators = [
            ind for ind in OPERATIONS_INDICATORS
            if ind['name'] in filtered_df.columns
        ]

        if not available_indicators:
            self.logger.warning("未找到任何企业经营指标")
            return pd.DataFrame()
    ...
    def create(
        ...
        try:
            ...
            # 创建2x2子图布局
            spacing = SUBPLOT_SPACING['2x2']
            fig = make_subplots(
                rows=2, cols=2,
                subplot_titles=[ind['title'] for ind in OPERATIONS_INDICATORS],
            ...
            # 为每个子图添加数据
            for idx, indicator in enumerate(OPERATIONS_INDICATORS):
                row = idx // 2 + 1
                col = idx % 2 + 1
            ...
            self.logger.error(f"创建企业经营指标图表时发生错误: {e}", exc_info=True)
            return None
