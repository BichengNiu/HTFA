...
def render_dfm_tab(st):
    ...
    # 从元数据获取指标
    # 获取因子数量（优先从标准键名，如未找到则从factor_loadings推断）
    k_factors = metadata.get('n_factors')
    if k_factors is None:
        ...
        logger.info(f"从元数据获取k_factors: {k_factors}")

    # 获取变量数量（优先从best_variables，如未找到则从factor_loadings推断）
