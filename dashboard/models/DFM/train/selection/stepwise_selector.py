...
class StepwiseSelector:
    ...
    def select(
        ...
        if progress_callback:
            ...
            progress_callback(
                f"初始变量: [{init_vars_str}] (训练期RMSE: {current_is_rmse:.4f}, 验证期RMSE: {current_oos_rmse:.4f})"
            )
        ...
        while remaining_vars:
            ...
            if progress_callback:
                progress_callback(
                    f"  前向：添加 '{best_add}' (验证期RMSE: {current_oos_rmse:.4f} -> {new_oos_rmse:.4f}, 改善: {delta_oos_rmse:+.4f})"
                )
    ...
    def _log_iteration_start(
        ...
        if progress_callback:
            ...
            progress_bar = self._generate_progress_bar(n_current, estimated_max_rounds)
    ...
    def _forward_step(
        ...
        progress_callback: Optional[Callable]
    ) -> Tuple[Optional[str], Tuple[float, float], int, int, float, float]:
        """
        前向步骤：找出最佳添加候选
    ...
    def _backward_step(
        ...
        progress_callback: Optional[Callable]
    ) -> Tuple[List[str], Tuple[float, float], int, int, float, float, List[str]]:
        """
        后向步骤：移除冗余变量
        ...
        while len(working_vars) > self.min_variables:
            ...
            if progress_callback:
                progress_callback(
                    f"  后向：移除 '{best_removal}' (验证期RMSE改善: {delta_oos_rmse:+.4f}, 剩余{len(working_vars)}个变量)"
                )
