...
提供数据输入参数映射、验证和转换功能
将外部输入（如UI层、API层）的参数适配为核心处理逻辑所需格式
"""
...
def map_input_variables_to_columns(
    ...
    Args:
        selected_variables: 输入的变量列表
        available_columns: 数据中可用的列名

    Returns:
        Dict[str, str]: 输入变量名到实际列名的映射
    """
    variable_mapping = {}

    for input_var in selected_variables:
        # 直接匹配
        if input_var in available_columns:
            variable_mapping[input_var] = input_var
            continue

        # 标准化匹配
        input_var_normalized = normalize_text(input_var)
        found_match = False

        for col in available_columns:
            col_normalized = normalize_text(col)
            if input_var_normalized == col_normalized:
                variable_mapping[input_var] = col
                found_match = True
                break

        if not found_match:
            print(f"    [WARNING] 警告: 输入变量 '{input_var}' 在数据中未找到匹配列")
...
def build_final_variable_list(
    ...
    for input_var, actual_col in variable_mapping.items():
...
def apply_stationarity_check(
    ...
    if skip_stationarity:
        print(f"  [数据准备] 跳过平稳性检查")
        return df, transform_details, removed_variables_log

    print(f"  [LOADING] [数据准备] 执行平稳性检查和转换...")
